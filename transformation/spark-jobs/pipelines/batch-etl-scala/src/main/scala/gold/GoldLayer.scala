...
object GoldLayer {
  ...
  // ============================================================
  // ORQUESTADOR GOLD — Modelos dimensionales y KPIs para BI
  // ============================================================
  def process(spark: SparkSession, silverPath: String, goldPath: String): Unit = {
    ...
    registerSilverTables(spark, silverPath)
  ...
  // ============================================================
  // REGISTRAR TABLAS SILVER COMO VISTAS SQL
  // ============================================================
  private def registerSilverTables(spark: SparkSession, silverPath: String): Unit = {
    val tables = Seq(
    ...
    tables.foreach { table =>
      ...
      if (DataLakeIO.pathExists(path)) {
        spark.read.parquet(path).createOrReplaceTempView(s"silver_$table")
        logger.info(s"  ✓ Vista registrada: silver_$table")
      } else {
  ...
  // ================================================================
  // RETAIL — dim_producto: Dimensión producto con métricas de negocio
  // Modelo Star Schema — Dimensión conformed para Power BI
  // ================================================================
  private def buildDimProducto(spark: SparkSession, goldPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        cp.Cod_Producto                                     AS producto_key,
        cp.Producto                                         AS producto_nombre,
        cp.Color                                            AS producto_color,
        cp.SubCategoria                                     AS subcategoria,
    ...
    DataLakeIO.writeDelta(df, goldPath, tableName)
    println(s"  ✔ gold/$tableName: ${df.count()} filas")
  }

  // ================================================================
  // RETAIL — dim_cliente: Dimensión cliente con segmentación y LTV
  // ================================================================
  private def buildDimCliente(spark: SparkSession, goldPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        Cod_Cliente                                         AS cliente_key,
        Segmento                                            AS segmento,
        Frecuencia                                          AS frecuencia_compras,
        Monetary                                            AS valor_monetario,
        Ticket_Promedio                                     AS ticket_promedio,
        Unidades_Totales                                    AS unidades_totales,
        Primera_Compra                                      AS fecha_primera_compra,
        Ultima_Compra                                       AS fecha_ultima_compra,
        Dias_Como_Cliente                                   AS antiguedad_dias,
        CASE
          WHEN Dias_Como_Cliente > 0
          THEN ROUND(Monetary / (Dias_Como_Cliente / 365.0), 2)
          ELSE Monetary
        END                                                 AS ltv_anualizado,
        CASE
          WHEN Frecuencia >= 10 THEN 5
          WHEN Frecuencia >= 5  THEN 4
  ...
  // ================================================================
  // RETAIL — fact_ventas: Tabla de hechos para análisis BI
  // Star Schema — Grano: línea de venta con claves a dimensiones
  // ================================================================
  private def buildFactVentas(spark: SparkSession, goldPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        ve.NumeroOrden                                      AS orden_id,
        ve.Cod_Producto                                     AS producto_key,
        ve.Cod_Cliente                                      AS cliente_key,
        ve.Cod_Territorio                                   AS territorio_key,
        YEAR(ve.FechaOrden)                                 AS anio,
        MONTH(ve.FechaOrden)                                AS mes,
        QUARTER(ve.FechaOrden)                              AS trimestre,
        DATE_FORMAT(ve.FechaOrden, 'yyyy-MM')               AS periodo,
        ve.FechaOrden                                       AS fecha_orden,
        ve.Cantidad                                         AS cantidad,
        ve.PrecioUnitario                                   AS precio_unitario,
        ve.CostoUnitario                                    AS costo_unitario,
        ve.Ingreso_Bruto                                    AS ingreso_bruto,
        ve.Costo_Total                                      AS costo_total,
        ve.Margen_Bruto                                     AS margen_bruto,
        ve.Pct_Margen                                       AS pct_margen,
        ve.Ganancia_Neta                                    AS ganancia_neta,
        ve.Impuesto                                         AS impuesto,
        ve.Flete                                            AS flete,
        ve.Dias_Envio                                       AS dias_envio,
        ve.Tipo_Envio                                       AS tipo_envio,
        ve.Tiene_Promocion                                  AS tiene_promocion,
        ve.Categoria                                        AS categoria,
        ve.SubCategoria                                     AS subcategoria,
    ...
    DataLakeIO.writeDelta(df, goldPath, tableName, partitionCol = Some("anio"))
    println(s"  ✔ gold/$tableName: ${df.count()} filas")
  }

  // ================================================================
  // RETAIL — kpi_ventas_mensuales: KPIs ejecutivos mensuales
  // Métricas pre-calculadas con variaciones MoM y acumulados YTD
  // ================================================================
    ...
    val df = spark.sql("""
      ...
      con_lag AS (
        SELECT
          ...
          SUM(Ingreso_Bruto) OVER (
            PARTITION BY Anio, Categoria ORDER BY Mes
            ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
          )                                                                      AS ingreso_ytd,
          SUM(Margen_Bruto) OVER (
            PARTITION BY Anio, Categoria ORDER BY Mes
            ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
          )                                                                      AS margen_ytd
        FROM base b
      ...
      SELECT
        ...
        ROUND(CASE
          ...
          THEN (Margen_Bruto / Ingreso_Bruto) * 100
          ELSE 0
        END, 2)                                             AS pct_margen,
        ROUND(CASE
          WHEN ingreso_mes_anterior > 0
          THEN ((Ingreso_Bruto - ingreso_mes_anterior) / ingreso_mes_anterior) * 100
          ELSE NULL
        END, 2)                                             AS variacion_mom_pct,
        ROUND(CASE
          WHEN ordenes_mes_anterior > 0
          THEN ((Total_Ordenes - ordenes_mes_anterior) * 1.0 / ordenes_mes_anterior) * 100
          ELSE NULL
        END, 2)                                             AS variacion_ordenes_mom_pct,
        ROUND(ingreso_ytd, 2)                               AS ingreso_ytd,
        ROUND(margen_ytd, 2)                                AS margen_ytd,
        current_timestamp()                                 AS _gold_updated_at
      FROM con_lag
  ...
  // ================================================================
  // MINING — dim_operador: Dimensión operador con ranking de eficiencia
  // ================================================================
  private def buildDimOperador(spark: SparkSession, goldPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        OperatorID                                          AS operador_key,
        FirstName                                           AS nombre,
        LastName                                            AS apellido,
        CONCAT(FirstName, ' ', LastName)                    AS nombre_completo,
        Country                                             AS pais,
        Trucks_Operados                                     AS trucks_operados,
        Proyectos                                           AS proyectos_asignados,
        Total_Operaciones                                   AS total_operaciones,
        Total_Mineral                                       AS total_mineral_extraido,
        Total_Desperdicio                                   AS total_desperdicio,
  ...
  // ================================================================
  // MINING — fact_produccion_minera: Hechos de producción diaria
  // ================================================================
  private def buildFactProduccionMinera(spark: SparkSession, goldPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        em.TruckID                                          AS truck_key,
        em.ProjectID                                        AS proyecto_key,
        em.Operaciones                                      AS total_operaciones,
        em.Total_Mineral                                    AS total_mineral,
        em.Total_Desperdicio                                AS total_desperdicio,
        em.Produccion_Neta                                  AS produccion_neta,
        em.Pct_Desperdicio                                  AS pct_desperdicio,
        em.Promedio_Mineral                                 AS promedio_mineral,
        em.StdDev_Mineral                                   AS stddev_mineral,
        em.Eficiencia                                       AS nivel_eficiencia,
        CASE
          WHEN em.StdDev_Mineral IS NOT NULL AND em.Promedio_Mineral > 0
          THEN ROUND(em.StdDev_Mineral / em.Promedio_Mineral * 100, 2)
          ELSE 0
        END                                                 AS coef_variacion_pct,
        pp.Total_Mineral                                    AS mineral_pais_total,
        CASE
          WHEN pp.Total_Mineral > 0
          THEN ROUND(em.Total_Mineral / pp.Total_Mineral * 100, 2)
          ELSE 0
        END                                                 AS pct_contribucion_pais,
        current_timestamp()                                 AS _gold_updated_at
      FROM silver_eficiencia_minera em
      LEFT JOIN silver_produccion_por_pais pp
        ON 1 = 1
    """)
  ...
  // ================================================================
  // MINING — kpi_mineria: KPIs ejecutivos de operación minera
  // ================================================================
  private def buildKpiMineria(spark: SparkSession, goldPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        pp.Country                                          AS pais,
        pp.Operadores                                       AS total_operadores,
        pp.Trucks                                           AS total_trucks,
        pp.Proyectos                                        AS total_proyectos,
        pp.Total_Registros                                  AS total_registros,
        pp.Total_Mineral                                    AS total_mineral,
        pp.Total_Desperdicio                                AS total_desperdicio,
        pp.Produccion_Neta                                  AS produccion_neta,
        pp.Promedio_Mineral                                 AS promedio_mineral_registro,
        ...
        current_timestamp()                                 AS _gold_updated_at
      FROM silver_produccion_por_pais pp
