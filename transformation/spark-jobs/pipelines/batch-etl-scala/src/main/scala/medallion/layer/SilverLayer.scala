...
object SilverLayer {
  ...
  private def buildCatalogoProductos(spark: SparkSession, silverPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        p.Cod_Producto, p.Producto, p.Color,
        sc.Cod_SubCategoria, sc.SubCategoria,
  ...
  private def buildVentasEnriquecidas(spark: SparkSession, silverPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        v.NumeroOrden, v.Cod_Cliente, v.Cod_Territorio, v.Cod_Producto,
        p.Producto, sc.SubCategoria, c.Categoria, p.Color,
  ...
  private def buildResumenVentasMensuales(spark: SparkSession, silverPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        YEAR(v.FechaOrden)    AS Anio, MONTH(v.FechaOrden) AS Mes, c.Categoria,
        ...
        ROUND(SUM(v.Cantidad * v.PrecioUnitario - v.Cantidad * v.CostoUnitario), 2) AS Margen_Bruto,
  ...
  private def buildRentabilidadProducto(spark: SparkSession, silverPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        p.Cod_Producto, p.Producto, p.Color, sc.SubCategoria, c.Categoria,
    ...
    val df = spark.sql("""
      WITH cliente_metricas AS (
        SELECT
          ...
          MAX(FechaOrden) AS Ultima_Compra, MIN(FechaOrden) AS Primera_Compra,
          DATEDIFF(MAX(FechaOrden), MIN(FechaOrden)) AS Dias_Como_Cliente,
          ROUND(AVG(Cantidad * PrecioUnitario), 2) AS Ticket_Promedio,
          SUM(Cantidad) AS Unidades_Totales
        FROM ventasinternet GROUP BY Cod_Cliente
      )
      SELECT Cod_Cliente, Frecuencia, Monetary, Ticket_Promedio, Unidades_Totales,
        Ultima_Compra, Primera_Compra, Dias_Como_Cliente,
        CASE
          ...
          ELSE 'Ocasional'
        END AS Segmento
      FROM cliente_metricas ORDER BY Monetary DESC
    """)
    DataLakeIO.writeParquet(df, silverPath, tableName)
    println(s"  ✔ silver/$tableName")
  }
  ...
  private def buildProduccionOperador(spark: SparkSession, silverPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        OperatorID, FirstName, LastName, Country,
        COUNT(DISTINCT TruckID) AS Trucks_Operados, COUNT(DISTINCT ProjectID) AS Proyectos,
      ...
      FROM mine GROUP BY OperatorID, FirstName, LastName, Country
  ...
  private def buildEficienciaMinera(spark: SparkSession, silverPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        f.TruckID, f.ProjectID, COUNT(*) AS Operaciones,
        ...
        CASE
          ...
          ELSE 'Baja'
        END AS Eficiencia
      FROM factmine f GROUP BY f.TruckID, f.ProjectID
  ...
  private def buildProduccionPorPais(spark: SparkSession, silverPath: String): Unit = {
    ...
    val df = spark.sql("""
      SELECT
        Country, COUNT(DISTINCT OperatorID) AS Operadores,
        COUNT(DISTINCT TruckID) AS Trucks, COUNT(DISTINCT ProjectID) AS Proyectos,
      ...
      FROM mine GROUP BY Country ORDER BY Total_Mineral DESC
    """)
    DataLakeIO.writeParquet(df, silverPath, tableName)
    println(s"  ✔ silver/$tableName")
  }
